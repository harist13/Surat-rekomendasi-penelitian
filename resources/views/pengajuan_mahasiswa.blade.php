            ...
            <div class="lg:w-2/3">
                <div class="bg-white rounded-lg shadow-md p-6 mb-6">
                    ...
                    <form action="{{ route('pengajuanmahasiswa.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        @if(isset($mahasiswa) && isset($rejectionReason))
                        ...
                        @if(isset($mahasiswa))
                            <input type="hidden" name="existing_id" value="{{ $mahasiswa->id }}">
                            <input type="hidden" name="no_pengajuan" value="{{ $mahasiswa->no_pengajuan }}">
                        @endif

                        <!-- Personal Data with Prefill -->
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="nama_lengkap" class="w-full p-2 border rounded-md" required value="{{ $mahasiswa->nama_lengkap ?? old('nama_lengkap') }}">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium mb-1">NIM<span class="text-red-500">*</span></label>
                                    <input type="text" name="nim" class="w-full p-2 border rounded-md" required value="{{ $mahasiswa->nim ?? old('nim') }}">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium mb-1">Email<span class="text-red-500">*</span></label>
                                    <input type="email" name="email" class="w-full p-2 border rounded-md" required value="{{ $mahasiswa->email ?? old('email') }}">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium mb-1">No. HP<span class="text-red-500">*</span></label>
                                    <input type="tel" name="no_hp" class="w-full p-2 border rounded-md" required value="{{ $mahasiswa->no_hp ?? old('no_hp') }}">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium mb-1">Alamat Peneliti<span class="text-red-500">*</span></label>
                                    <textarea name="alamat_peneliti" class="w-full p-2 border rounded-md" rows="3" required>{{ $mahasiswa->alamat_peneliti ?? old('alamat_peneliti') }}</textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <input type="text" name="nama_instansi" class="w-full p-2 border rounded-md" required value="{{ $mahasiswa->nama_instansi ?? old('nama_instansi') }}">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium mb-1">Alamat Instansi<span class="text-red-500">*</span></label>
                                    <textarea name="alamat_instansi" class="w-full p-2 border rounded-md" rows="3" required>{{ $mahasiswa->alamat_instansi ?? old('alamat_instansi') }}</textarea>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium mb-1">Fakultas/Jurusan<span class="text-red-500">*</span></label>
                                    <input type="text" name="jurusan" class="w-full p-2 border rounded-md" required value="{{ $mahasiswa->jurusan ?? old('jurusan') }}">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <textarea name="judul_penelitian" class="w-full p-2 border rounded-md" rows="3" required>{{ $mahasiswa->judul_penelitian ?? old('judul_penelitian') }}</textarea>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium mb-1">Lama Penelitian<span class="text-red-500">*</span></label>
                                    <input type="text" name="lama_penelitian" class="w-full p-2 border rounded-md" required value="{{ $mahasiswa->lama_penelitian ?? old('lama_penelitian') }}">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium mb-1">Tanggal Mulai<span class="text-red-500">*</span></label>
                                    <input type="date" name="tanggal_mulai" class="w-full p-2 border rounded-md" required value="{{ $mahasiswa->tanggal_mulai ?? old('tanggal_mulai') }}">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium mb-1">Tanggal Selesai<span class="text-red-500">*</span></label>
                                    <input type="date" name="tanggal_selesai" class="w-full p-2 border rounded-md" required value="{{ $mahasiswa->tanggal_selesai ?? old('tanggal_selesai') }}">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium mb-1">Lokasi Penelitian<span class="text-red-500">*</span></label>
                                    <textarea name="lokasi_penelitian" class="w-full p-2 border rounded-md" rows="3" required>{{ $mahasiswa->lokasi_penelitian ?? old('lokasi_penelitian') }}</textarea>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium mb-1">Tujuan Penelitian<span class="text-red-500">*</span></label>
                                    <input type="text" name="tujuan_penelitian" class="w-full p-2 border rounded-md" required value="{{ $mahasiswa->tujuan_penelitian ?? old('tujuan_penelitian') }}">
                                </div>
                                <div class="w-full">
                                    <label class="block text-sm font-medium mb-1">Anggota peneliti<span class="text-red-500">*</span></label>
                                    <textarea name="anggota_peneliti" class="p-2 border rounded-md w-full sm:min-w-[400px] md:min-w-[600px] lg:min-w-[760px]" rows="5" required>{{ $mahasiswa->anggota_peneliti ?? old('anggota_peneliti') }}</textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    @if(isset($mahasiswa) && $mahasiswa->surat_pengantar_instansi)
                                        <div class="flex items-center mt-2">
                                            <p class="text-xs text-gray-500">File sebelumnya: </p>
                                            <a href="{{ Storage::url($mahasiswa->surat_pengantar_instansi) }}" target="_blank" class="ml-2 text-xs text-blue-600 hover:text-blue-800 hover:underline flex items-center">
                                ...
                                <div>
                                    ...
                                    @if(isset($mahasiswa) && $mahasiswa->proposal_penelitian)
                                        <div class="flex items-center mt-2">
                                            <p class="text-xs text-gray-500">File sebelumnya: </p>
                                            <a href="{{ Storage::url($mahasiswa->proposal_penelitian) }}" target="_blank" class="ml-2 text-xs text-blue-600 hover:text-blue-800 hover:underline flex items-center">
                                                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                                </svg>
                                ...
                                <div>
                                    ...
                                    @if(isset($mahasiswa) && $mahasiswa->ktp)
                                        <div class="flex items-center mt-2">
                                            <p class="text-xs text-gray-500">File sebelumnya: </p>
                                            <a href="{{ Storage::url($mahasiswa->ktp) }}" target="_blank" class="ml-2 text-xs text-blue-600 hover:text-blue-800 hover:underline flex items-center">
                                                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                                </svg>
